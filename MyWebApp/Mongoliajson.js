...
.then(data => {
  ...
  function getColor(scientific_name) {
    switch (scientific_name) {
      ...
      case "Nothocricetulus migratorius":
...
$('#range').multiselect({
  placeholder: 'Select Species Range',
  search: true,
  selectAll: true,
  selectGroup: true,
  maxPlaceholderOpts: 1,
});
function getColor(SCI_NAME) {
  switch (SCI_NAME) {
    case "Allocricetulus curtatus":
      return {color: "rgba(0, 0, 0, 1.0)", stroke: "rgba(105, 1, 0)"}; //  black with red outline
    case "Alticola":
    ...
    .then(data => {
      ...
      geoJSONLayer = L.geoJSON(filteredFeatures, {
        style: function(feature) {
          var color = getColor(feature.properties.SCI_NAME);
          return {
            fillColor: color.color,
            color: color.stroke,
            weight: 2,
